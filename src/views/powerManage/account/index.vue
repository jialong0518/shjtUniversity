<template>
  <div class="account">
    ...
    <div style="padding: 15px; overflow: hidden">
      <plupload @updata="batchImport" style="float: right"
        >导入秘书信息</plupload
      >
      <el-button
        type="primary"
    ...
    >
      <div style="font-size: 18px">
        账号：<span style="font-size: 18px; font-weight: 600">{{
          account
        }}</span>
      </div>
      <div style="font-size: 18px">
        密码：<span style="font-size: 18px; font-weight: 600">{{ word }}</span>
      </div>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      rulesAccount: {
        ...
        role: [{ required: true, message: "请填选择角色", trigger: "change" }],
        faculty: [
          { required: true, message: "请填选择院系", trigger: "change" },
        ],
        email: [{ required: true, message: "请填写email", trigger: "blur" }],
        code: [{ required: true, message: "请填写学工号", trigger: "blur" }],
      },
  ...
  methods: {
    ...
    downFile() {
      download(
        "https://mob.hexntc.com/expert/downloadfile?file=secretary.xlsx",
        "secretary.xlsx"
      );
    ...
    getTableData() {
      userlist({
        id: this.searchID === "" ? 0 : Number(this.searchID),
